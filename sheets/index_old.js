...
fs.readFile("credentials.json", (err, content) => {
  ...
  authorize(JSON.parse(content), workWithMySpreadsheet);
});
...
async function workWithMySpreadsheet(oAuth2Client) {
  ...
  try {
    const { data } = await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: "MySheet1!A4:C4",
      valueInputOption: "RAW",
      resource: {
        values: [["2", "Дела довольно неплохо!!", "23.07.2020"]]
      }
    });
